...
namespace Outsourcing.Service
{
    ...
    public class BlogService : IBlogService
    {
        ...
        {
            var blogs = blogRepository.GetMany(p=>p.Deleted == false);
            return blogs;
        }
        ...
        {
            var blogs = this.GetBlogs().Where(p => p.BlogCategoryId == 9 && p.IsAvailable == true && p.Deleted == false).OrderBy(p=>p.DateCreated).Take(3);
            return blogs;
        }
        #endregion

        public Blog GetBlogPromotion()
        {
           return blogRepository.GetMany(p => !p.Deleted && p.BlogCategoryId == 8).OrderByDescending(d => d.DateCreated).FirstOrDefault();
        }
        ...
        {
            return blogRepository.GetAll().Where(p => p.BlogCategoryId == 9).OrderByDescending(p => p.DateCreated);
        }
    }
}
